...
class Smooth_Calendar_Admin {
	...
	public function add_menu() {
		...
		add_options_page(
			apply_filters( $this->plugin_name . '-settings-page-title', __( 'Smooth Calendar Settings', '' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', __( 'Smooth Calendar', 'calendar' ) ),
			'manage_options',
			$this->plugin_name,
	...
	public function options_page() {
		?><div class="wrap"><h2><?php echo esc_html( get_admin_page_title() ); ?></h2>
	...
	public function register_settings() {
		...
		add_settings_field(
			'link-to',
			apply_filters( $this->plugin_name . '-link-to-label', __( 'Link events to new page?	', 'calendar' ) ),
			array( $this, 'link_to_field' ),
			$this->plugin_name,
			$this->plugin_name . '-display-options'
		);
	} // register_settings()
	...
	public function validate_options( $input ) {
		...
		if ( isset( $input['link-to'] ) ) {
			$display_salary 			= trim( $input['link-to'] );
			$valid['link-to'] 	= isset( $display_salary ) ? 1 : 0;
			if ( $valid['link-to'] != $input['link-to'] ) {
				add_settings_error( 'link-to', 'link_to_error', __( 'Link to error.', 'calendar' ), 'error' );
			}
		}
	 ...
	 */
	public function link_to_field() {
		$options 	= get_option( $this->plugin_name . '-options' );
		$option 	= 0;
		if ( ! empty( $options['link-to'] ) ) {
			$option = $options['link-to'];
		}
		?><input type="checkbox" id="<?php echo $this->plugin_name; ?>-options'[link-to]" name="<?php echo $this->plugin_name; ?>-options'[link-to]" value="1" <?php checked( 1, $option, false ); ?> />
		<p class="description">If checked, events listed on the calendar will link to their unique pages. Otherwise, a tooltip will be used to display the information.</p><?php
	} // link_to_field()
